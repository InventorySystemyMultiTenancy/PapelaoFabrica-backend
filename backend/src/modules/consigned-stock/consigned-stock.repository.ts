...
async function upsert(
  ...
  const existing = await pool.query<StockRow>(
    `SELECT * FROM consigned_stock WHERE client_id = $1 AND product_id = $2`,
    [input.clientId, input.productId ?? null],
  );
  if (existing.rows[0]) return rowToStock(existing.rows[0]);

  const id = randomUUID();
  const result = await pool.query<StockRow>(
    `INSERT INTO consigned_stock (id, client_id, product_id, product_name, notes)
