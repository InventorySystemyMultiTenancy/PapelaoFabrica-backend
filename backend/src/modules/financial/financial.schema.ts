...
export const generateInstallmentsSchema = z.object({
  ...
  totalAmount: z.number({ required_error: "totalAmount é obrigatório" }).positive("totalAmount deve ser maior que zero"),
  paymentType: z.enum(["avista", "parcelado"]),
  installmentDays: z
    .array(z.number().int().nonnegative("Os dias de parcela não podem ser negativos"))
...
export const updateReceivableSchema = z.object({
  paidAt: z
    .string()
    .trim()
    .refine((v) => !Number.isNaN(Date.parse(v)), { message: "paidAt deve ser uma data válida" })
    .optional()
    .nullable(),
  ...
  dueDate: z
    .string()
    .trim()
    .refine((v) => !Number.isNaN(Date.parse(v)), { message: "dueDate deve ser uma data válida" })
    .optional(),
